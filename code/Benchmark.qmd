...
```{r}
source("R/Scripts/adaptiveKNN.R")

# Randomly select indices to be set to NA
na_indices <- sample(1:nrow(full_umap), 200)

# Introduce NA values at the selected indices
ct_na <- full_umap$CellType

ct_na[na_indices] <- NA

names(ct_na) <- full_umap$Row.names


  knn_out = embeddingKNN(full_umap_coords,
                         ct_na,
                         type = "uniform_fixed",
                         k_values = 5)
```
