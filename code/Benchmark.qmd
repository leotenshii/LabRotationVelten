...
# List for StabMap
assay_list = list(
  ATAC = logcounts_all[assayType %in% c("atac"), names %in% c("ATAC")], # 788x5016 -> only ATAC in first half of cells
...
May be interesting: corrected.rand (Corrected Rand Index): Measures the similarity between the true labels and the clustering results. It accounts for chance groupings, making it a robust metric for comparing clustering performance against the ground truth.
...
Get labels of imputed values (k nearest neighbors), then F1

```{r}
# library(caret)
# 
# mofa_predcited <- mofa_imp_comp$predicted
# mofa_actual <- mofa_imp_comp$actual
# 
# confusionMatrix(predcited, actual,
#                 mode = "everything",
#                 positive="1")
# 
# confusionMatrix(predcited, actual,
